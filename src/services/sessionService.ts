 ...
 */
const createSession = (sequelize: Sequelize) => async (_id: string) => {

    // Delete any other session data for the user
    await sequelize.models.session.destroy({ where: { id: _id } });

    // Create new session for the user
    const { id, user_id } = await sequelize.models.session.create({ user_id: _id, }) as ISession;
